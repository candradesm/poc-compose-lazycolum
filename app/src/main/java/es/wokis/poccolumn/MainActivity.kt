...
fun Greeting(
    content: List<String>,
    staticElement: @Composable () -> Unit,
    dynamicElement: @Composable (element: String) -> Unit
) {
    LazyColumn {
        item {
            staticElement()
...
@Composable
private fun DynamicElement(it: String) {
    Image(
        painter = painterResource(id = R.drawable.ic_launcher_foreground),
        contentDescription = "pepito"
    )
    Text(text = it, modifier = Modifier.background(Color.Black), color = Color.White)
}

@Composable
private fun StaticElement(name: String, modifier: Modifier) {
    Text(
        text = "Hello $name!",
        modifier = modifier
    )
...
fun GreetingPreview() {
    PocColumnTheme {
        Greeting(listOf("ejemplo", "aaaaaaaa"), {}, {})
    }
}